...
For this fintech project, the model predicts whether a customer subscribe to a financial product. The system that makes that prediction reproducible, testable, deployable, and usable by another application.
...
The target column must be removed from X because the model should not be given the anwer as an input. 
